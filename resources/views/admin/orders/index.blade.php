...
<div class="space-y-8 px-4 md:px-0 pb-12">
    ...
    <div class="reveal-anim flex flex-col lg:flex-row items-center justify-between gap-4 bg-white/5 p-4 rounded-[28px] border border-white/10 shadow-2xl backdrop-blur-md" style="animation-delay: 0.2s">
        ...
        {{-- FORM FILTER --}}
        <form method="GET" action="{{ route('admin.orders.index') }}" class="flex flex-col sm:flex-row items-center gap-3 w-full lg:w-auto">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8 reveal-anim" style="animation-delay: 0.3s">
        ...
        <div class="row-anim group relative bg-[#111827]/40 backdrop-blur-md rounded-[50px] border border-white/5 p-8 hover:border-[#F0B22B]/30 transition-all duration-500 hover:-translate-y-2"
            ...
            <div class="flex items-center justify-between mb-8">
                <div class="bg-white/5 px-4 py-2 rounded-full border border-white/10">
                    <span class="text-gray-400 text-[10px] font-black tracking-widest italic uppercase">#{{ $order->id }}</span>
                </div>
                
                @php
                    $statusConfig = match($order->status) {
                        'selesai' => ['label' => 'COMPLETED', 'color' => 'text-green-400', 'bg' => 'bg-green-500/10', 'border' => 'border-green-500/20'],
                        'menunggu_verifikasi' => ['label' => 'VERIFYING', 'color' => 'text-blue-400', 'bg' => 'bg-blue-500/10', 'border' => 'border-blue-500/20'],
                        'menunggu_pembayaran_tunai' => ['label' => 'PENDING CASH', 'color' => 'text-yellow-400', 'bg' => 'bg-yellow-500/10', 'border' => 'border-yellow-500/20'],
                        'dibatalkan' => ['label' => 'CANCELLED', 'color' => 'text-red-400', 'bg' => 'bg-red-500/10', 'border' => 'border-red-500/20'],
                        default => ['label' => 'UNKNOWN', 'color' => 'text-gray-400', 'bg' => 'bg-gray-500/10', 'border' => 'border-gray-500/20'],
                    };
                @endphp
                <div class="flex items-center gap-2 {{ $statusConfig['bg'] }} {{ $statusConfig['color'] }} {{ $statusConfig['border'] }} border px-4 py-2 rounded-full">
            ...
            <div class="mb-8">
                <div class="flex flex-col">
                    <span class="text-[#F0B22B] text-[10px] font-black uppercase tracking-[0.3em] mb-1 opacity-80">Total Payment</span>
                    <h2 class="text-white text-2xl font-black italic tracking-tighter leading-none group-hover:scale-105 transition-transform duration-500 origin-left">
                        <span class="text-xl mr-1 not-italic opacity-50 font-bold">Rp</span>{{ number_format($order->total_harga,0,',','.') }}
                    </h2>
                </div>
                
                <div class="mt-6 space-y-2">
                    <h3 class="text-gray-200 text-lg font-black leading-tight line-clamp-1 italic group-hover:text-[#F0B22B] transition-colors">
                        {{ $order->nama_produk }}
                    </h3>
                    <div class="flex items-center gap-3">
                        <span class="text-gray-500 text-[10px] font-black tracking-widest uppercase">{{ $order->qty }} Units</span>
                        <div class="w-1 h-1 rounded-full bg-white/20"></div>
                        <span class="text-gray-500 text-[10px] font-black tracking-widest uppercase italic">{{ str_replace('_',' ',$order->metode_pembayaran) }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="flex items-center gap-4 p-5 bg-white/[0.03] rounded-[30px] border border-white/5 mb-8 hover:bg-white/[0.08] transition-all cursor-pointer group/user" 
                ...
                <div class="relative">
                    ...
                    <div class="absolute -bottom-1 -right-1 bg-green-500 w-4 h-4 rounded-full border-4 border-[#111827]"></div>
                </div>
                <div class="flex-1 min-w-0">
                    <p class="text-white text-sm font-black truncate tracking-wide">{{ $order->user->nama ?? 'Guest' }}</p>
                    <p class="text-gray-500 text-[10px] font-bold truncate tracking-widest uppercase opacity-70">{{ $order->user->email ?? 'N/A' }}</p>
                </div>
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#F0B22B" stroke-width="3" class="opacity-0 group-hover/user:opacity-100 transition-all -translate-x-4 group-hover/user:translate-x-0"><path d="m9 18 6-6-6-6"/></svg>
            </div>
            ...
            <div class="flex gap-4">
                @if($order->bukti_bayar)
                    <button onclick="toggleImage('{{ asset('storage/' . $order->bukti_bayar) }}')" 
                        ...
                        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#F0B22B" stroke-width="2.5"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path><circle cx="12" cy="12" r="3"></circle></svg>
                        <span class="text-[10px] font-black uppercase tracking-[0.2em]">View Proof</span>
                    </button>
                @else
                    <div class="flex-1 flex items-center justify-center gap-3 bg-white/5 opacity-30 text-gray-500 py-4 rounded-2xl border border-white/5 italic">
                ...
                @if($order->status !== 'dibatalkan')
                    <div class="relative group/status flex-1">
                        <div class="flex items-center justify-center gap-3 bg-[#F0B22B] hover:bg-[#ffc13b] text-[#090069] py-4 rounded-2xl transition-all shadow-lg shadow-[#F0B22B]/20 cursor-pointer active:scale-95">
                            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3"><path d="M12 20h9M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"></path></svg>
                            <span class="text-[10px] font-black uppercase tracking-[0.2em]">Update</span>
                        </div>
                        
                        <div class="status-dropdown">
                            <form action="{{ route('admin.orders.status', $order->id) }}" method="POST" class="bg-[#1a203c] border border-white/10 rounded-[25px] shadow-2xl overflow-hidden p-2">
                                @csrf
                                <button name="status" value="menunggu_pembayaran_tunai" class="w-full text-left px-5 py-3.5 text-[10px] font-black uppercase text-yellow-400 hover:bg-yellow-400/10 rounded-xl transition-colors mb-1">● Cash Pending</button>
                                <button name="status" value="menunggu_verifikasi" class="w-full text-left px-5 py-3.5 text-[10px] font-black uppercase text-blue-400 hover:bg-blue-400/10 rounded-xl transition-colors mb-1">● Verifying</button>
                                <button name="status" value="selesai" class="w-full text-left px-5 py-3.5 text-[10px] font-black uppercase text-green-400 hover:bg-green-400/10 rounded-xl transition-colors">● Completed</button>
                            </form>
                        </div>
                    </div>
                @else
                    <div class="flex-1 flex items-center justify-center gap-3 bg-red-500/5 text-red-500/50 py-4 rounded-2xl border border-red-500/10 cursor-not-allowed">
                        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"></rect><path d="M7 11V7a5 5 0 0 1 10 0v4"></path></svg>
        ...
        <div class="col-span-full py-32 text-center">
            <div class="inline-block p-16 bg-white/5 rounded-[60px] border border-white/5 relative overflow-hidden">
                <div class="absolute inset-0 bg-gradient-to-br from-[#F0B22B]/5 to-transparent"></div>
                <svg xmlns="http://www.w3.org/2000/svg" width="80" height="80" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1" class="mx-auto text-gray-700 mb-6 opacity-30"><rect x="2" y="7" width="20" height="14" rx="2" ry="2"></rect><path d="M16 21V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v16"></path></svg>
                <p class="text-gray-500 font-black uppercase tracking-[0.5em] text-sm relative z-10">No Transactions Found</p>
            </div>
        </div>
        @endforelse
    </div>
...
<div id="userModal" class="fixed inset-0 z-[9999] hidden flex items-center justify-center p-4">
    ...
    <div class="bg-[#161B33] border border-white/10 w-full max-w-xl rounded-[60px] p-10 md:p-14 relative z-10 shadow-2xl transform transition-all duration-500 scale-90 opacity-0" id="modalContent">
        <button onclick="closeModal()" class="absolute top-12 right-12 text-gray-500 hover:text-[#F0B22B] transition-all hover:rotate-90">
        ...
        <div class="flex flex-col items-center text-center">
            <div class="relative mb-8">
                ...
                <img id="modalFoto" src="" class="relative w-40 h-40 rounded-[45px] border-4 border-[#F0B22B]/20 object-cover shadow-2xl">
            </div>
            <h2 id="modalNama" class="text-white text-3xl font-black mb-2 tracking-tight"></h2>
            <p id="modalUsername" class="text-[#F0B22B] text-sm font-black italic mb-12 tracking-[0.2em] uppercase opacity-80"></p>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5 w-full">
                <div class="bg-white/5 p-6 rounded-[30px] border border-white/5 text-left">
                    <p class="text-[#F0B22B] text-[10px] font-black uppercase tracking-widest mb-2 opacity-50">E-Mail</p>
                    <p id="modalEmail" class="text-white text-sm font-black truncate"></p>
                </div>
                <div class="bg-white/5 p-6 rounded-[30px] border border-white/5 text-left">
                    <p class="text-[#F0B22B] text-[10px] font-black uppercase tracking-widest mb-2 opacity-50">WhatsApp</p>
            ...
            <div class="w-full mt-5 bg-white/5 p-6 rounded-[30px] border border-white/5 text-left">
                <p class="text-[#F0B22B] text-[10px] font-black uppercase tracking-widest mb-2 opacity-50">Resident Address</p>
...
.custom-pagination nav a { 
    border-radius: 12px !important; /* Lebih kotak dikit biar rapi */
    background: rgba(255,255,255,0.03) !important; 
    color: #6b7280 !important; /* Abu-abu biar ga kontras banget */
    border: 1px solid rgba(255,255,255,0.08) !important; 
    padding: 8px 14px !important; /* Ukuran slim */
    font-size: 11px !important; /* Font lebih kecil */
    font-weight: 800 !important;
    margin: 0 2px !important; /* Jarak antar tombol lebih rapet */
...
.img-overlay { position: fixed; inset: 0; background: rgba(9, 0, 105, 0.98); backdrop-filter: blur(25px); z-index: 99999; display: flex; align-items: center; justify-content: center; cursor: zoom-out; }
.img-overlay img { max-width: 90%; max-height: 85%; border-radius: 40px; border: 4px solid #F0B22B; shadow: 0 0 120px rgba(240,178,43,0.3); }
</style>
