...
<div class="max-w-4xl mx-auto px-4 pb-20">
    ...
    <div class="mb-8 flex items-center justify-between reveal-anim" style="animation-delay: 0.1s">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h2 class="text-white text-xl md:text-2xl font-bold tracking-tight">Edit Product</h2>
                <p class="text-gray-400 text-[10px] md:text-xs uppercase tracking-widest mt-1">Update Serbu Computer Inventory</p>
            </div>
        </div>
    </div>

    {{-- CARD FORM DENGAN ANIMASI ZOOM-IN --}}
    <div class="reveal-anim bg-white/5 backdrop-blur-md rounded-[24px] md:rounded-[28px] border border-white/10 p-5 md:p-8 shadow-2xl" style="animation-delay: 0.2s">
        <form action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-6">
                <div class="space-y-2">
                    ...
                    <input type="text" name="name" value="{{ old('name', $product->name) }}" 
                           ...
                           placeholder="Laptop/Accessory name">
                </div>
                <div class="space-y-2">
                    ...
                    <div class="relative">
                        <select name="category" class="w-full bg-[#090069]/40 border border-white/10 rounded-2xl px-5 py-3 md:py-3.5 text-sm md:text-base text-white focus:outline-none focus:border-[#F0B22B] appearance-none cursor-pointer">
                            ...
                            <option value="Aksesoris" @selected($product->category == 'Aksesoris') class="bg-[#090069]">Accessories</option>
                        </select>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5 md:gap-6">
                ...
                <div class="space-y-2">
                    <label class="text-[#F0B22B] text-[10px] md:text-xs font-bold uppercase tracking-wider ml-1">Stock Available</label>
            ...
            <div class="space-y-6">
                ...
                <div class="space-y-2">
                    <label class="text-[#F0B22B] text-[10px] md:text-xs font-bold uppercase tracking-wider ml-1">Product Details</label>
                ...
                <div class="flex flex-col md:flex-row items-center md:items-start gap-5 md:gap-8">
                    @if($product->photo)
                        <div class="relative group shrink-0">
                            ...
                            <span class="absolute -top-2 -right-2 bg-[#F0B22B] text-[#090069] text-[9px] font-bold px-2 py-0.5 rounded-full shadow-md">ACTIVE</span>
                        </div>
                    @endif
                    <div class="flex-grow w-full text-center md:text-left">
                        <p class="text-white font-bold mb-3 text-sm md:text-base">Upload New Photo</p>
                        ...
                        <p class="text-[9px] md:text-[10px] text-gray-500 mt-3 uppercase tracking-tighter">*Leave empty if you don't want to change photo</p>
                    </div>
                </div>
            </div>
...
/* 2. STYLE INPUT FOCUS (Opsional tapi mempercantik) */
input:focus, textarea:focus, select:focus {
    box-shadow: 0 0 15px rgba(240, 178, 43, 0.1);
}
</style>
@endsection