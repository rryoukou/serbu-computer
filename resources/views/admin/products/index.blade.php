...
@section('content')
    <div class="space-y-8 px-4 md:px-0">
        ...
        @if (session('success'))
            <div class="reveal-anim bg-green-500/10 border border-green-500/50 text-green-400 px-6 py-4 rounded-2xl flex items-center gap-3"
        ...
        {{-- TABLE SECTION --}}
        <div class="reveal-anim bg-white/5 backdrop-blur-md rounded-[32px] border border-white/10 overflow-hidden shadow-2xl"
                            ...
                            <tr class="hover:bg-white/[0.02] transition-colors group">
                                <td class="px-8 py-5">
                                    <div class="flex justify-center">
                                        <div
                                            class="w-16 h-16 rounded-2xl overflow-hidden border-2 border-white/10 group-hover:border-[#F0B22B]/50 transition-all shadow-lg">
                                            @if ($product->photo)
                                                <img src="{{ asset('storage/' . $product->photo) }}"
                                                    alt="{{ $product->name }}" class="w-full h-full object-cover">
                                            @else
                                                <div
                                                    class="w-full h-full bg-[#161B33] flex items-center justify-center text-gray-600 text-[10px]">
                                                    No Image</div>
                                            @endif
                                        </div>
                                    </div>
                                ...
                                <td class="px-8 py-5">
                                    <div
                                        class="font-bold text-white text-base group-hover:text-[#F0B22B] transition-colors">
                                        {{ $product->name }}</div>
                                    <div class="text-[#F0B22B] font-black italic text-sm mt-0.5">Rp
                                        {{ number_format($product->price, 0, ',', '.') }}</div>
                                </td>
                                <td class="px-8 py-5">
                                    <span
                                        class="px-3 py-1 bg-white/5 rounded-full text-gray-400 text-[10px] font-black uppercase tracking-widest border border-white/10">{{ $product->category }}</span>
                                </td>
                                <td class="px-8 py-5 text-center">
                                    <span
                                        class="font-black {{ $product->stock <= 5 ? 'text-red-400 animate-pulse' : 'text-white' }}">{{ $product->stock }}</span>
                                </td>
                                <td class="px-8 py-5 text-center">
            ...
            <div class="md:hidden divide-y divide-white/5">
                @forelse($products as $product)
                    <div class="p-6 flex items-center gap-5 group">
                        <div
                            class="w-20 h-20 shrink-0 rounded-2xl overflow-hidden border-2 border-white/10 group-hover:border-[#F0B22B]/50 transition-all">
                            @if ($product->photo)
                                <img src="{{ asset('storage/' . $product->photo) }}" class="w-full h-full object-cover">
                            @else
                                <div
                                    class="w-full h-full bg-[#161B33] flex items-center justify-center text-gray-600 text-[8px]">
                                    No Image</div>
                            @endif
                        </div>
                        <div class="flex-grow space-y-1">
                            <div class="font-bold text-white group-hover:text-[#F0B22B] transition-colors line-clamp-1">
                                {{ $product->name }}</div>
                            <div class="text-[#F0B22B] font-black text-xs italic">Rp
                                {{ number_format($product->price, 0, ',', '.') }}</div>
                            <div class="flex items-center gap-2 pt-1">
                                <span class="text-[9px] font-bold text-gray-500 uppercase">{{ $product->category }}</span>
                                <span
                                    class="text-[9px] font-bold {{ $product->stock <= 5 ? 'text-red-400' : 'text-gray-400' }}">Stock:
                                    {{ $product->stock }}</span>
                            </div>
                        </div>
                        <a href="{{ route('admin.products.edit', $product->id) }}"
                            class="w-10 h-10 bg-yellow-500/10 text-yellow-500 rounded-xl flex items-center justify-center">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24"
                                fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                                stroke-linejoin="round">
                                <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path>
