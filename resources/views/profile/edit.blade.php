...
@section('content')
<div class="min-h-screen bg-[#090069] py-12 px-4">
    <div class="max-w-5xl mx-auto">
        
        {{-- Header Page --}}
        <div class="mb-8 flex items-center justify-between">
            <h2 class="text-3xl font-bold text-white tracking-wide">Profil Saya</h2>
        ...
        {{-- Alerts --}}
        @if (session('success'))
            <div class="mb-6 px-6 py-4 bg-green-500/20 border border-green-500 text-green-200 rounded-xl backdrop-blur-md">
                {{ session('success') }}
            </div>
        @endif

        @if ($errors->any())
            <div class="mb-6 px-6 py-4 bg-red-500/20 border border-red-500 text-red-200 rounded-xl backdrop-blur-md">
                {{ $errors->first() }}
            </div>
        @endif

        {{-- MAIN CARD --}}
        <div class="bg-[#000066] rounded-[30px] shadow-2xl overflow-hidden border border-white/10 p-6 md:p-12">
            <form method="POST" action="{{ route('profile.update') }}" enctype="multipart/form-data">
                @csrf

                <div class="flex flex-col lg:flex-row gap-12">
                    
                    {{-- SISI ATAS/KANAN: FOTO PROFIL --}}
                    {{-- Sekarang muncul di atas pada Mobile (order-first) dan di kanan pada Desktop (lg:order-last) --}}
                    <div class="w-full lg:w-[300px] flex flex-col items-center justify-start space-y-6 order-first lg:order-last">
                        <div class="relative group">
                            {{-- Frame Foto --}}
                            <div class="w-40 h-40 md:w-56 md:h-56 bg-white rounded-full p-1 shadow-2xl overflow-hidden flex items-center justify-center border-4 border-[#F0B22B]/30">
                                @if ($user->foto)
                        ...
                        <div class="text-center">
                            ...
                            <p class="text-gray-400 text-xs px-4 mt-1">
                                Klik ikon kamera untuk mengganti foto profil.
                            </p>
                        </div>
                    </div>

                    {{-- SISI BAWAH/KIRI: INPUT FIELDS --}}
                    <div class="flex-1 space-y-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    <select name="jenis_kelamin"
                                        class="w-full bg-white rounded-2xl px-5 py-3 text-gray-800 focus:ring-4 focus:ring-[#F0B22B]/50 outline-none transition-all shadow-inner appearance-none">
                            ...
                            {{-- No HP --}}
                            <div class="md:col-span-2">
                                <label class="block text-gray-300 text-sm font-medium mb-2 ml-1">No HP</label>
    ...
    <textarea name="alamat" rows="3"
        ...
        placeholder="Masukkan alamat lengkap">{{ old('alamat', $user->alamat) }}</textarea>
                        ...
                        <div class="pt-4 border-t border-white/10 mt-6">
                            ...
                            <p class="mt-2 text-xs text-gray-400 ml-1 italic">*Kosongkan jika tidak ingin mengganti password</p>
                        </div>

                        {{-- Tombol Simpan (Mobile & Desktop) --}}
                        <div class="pt-8">
                            <button type="submit"
                                class="w-full bg-[#F0B22B] hover:bg-yellow-400 text-black font-extrabold px-6 py-4 rounded-2xl transition duration-300 transform hover:-translate-y-1 shadow-[0_10px_20px_rgba(240,178,43,0.3)] uppercase tracking-widest text-sm">
                                Simpan Perubahan
                            </button>
                        </div>
                    </div>
...
<a href="{{ route('dashboard') }}"
   class="fixed bottom-6 left-6 z-50 hidden md:inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-[#0c0c3d] border border-gray-700 text-sm font-medium text-gray-200 shadow-2xl hover:border-[#F0B22B] hover:text-[#F0B22B] transition-all duration-300 group">
